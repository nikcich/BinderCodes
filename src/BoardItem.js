...
const BoardItem = (props) => {
    ...
    const deleteImage = (users_id, location) => {
        axios.post('http://api.cichosz.dev/delete/image', {id: users_id, location: location}).then(result => {
        });
    }

    const deleteUser = (users_id, location) => {
        axios.post('http://api.cichosz.dev/delete', {id: users_id, location: location}).then(result => {
        });
    }

    const resetUser = (users_id, location) => {
        axios.post('http://api.cichosz.dev/reset', {id: users_id, location: location}).then(result => {
        });
    }
                ...
                <div className="lowerdivision">
                    <div className="boardImageHolder">
                        <div className="boardImage"> 
                            <img src={'https://api.cichosz.dev/getimage/'+item.users_id} id="displayImg1" className="displayImg1" alt="Loading..."></img>
                        </div>
                    </div>
                </div>
                <div  className="lowerdivision">
